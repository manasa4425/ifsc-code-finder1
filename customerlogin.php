...
if (isset($_POST['login'])) {
    $accno = mysqli_real_escape_string($conn, $_POST['account_no']);
    $pin   = mysqli_real_escape_string($conn, $_POST['pin']);

    $sql = "SELECT * FROM account WHERE accno='$accno' AND pin='$pin'";
...
if (isset($_POST['forgot'])) {
    ...
    if ($result && mysqli_num_rows($result) == 1) {
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'myprojectemails4u@gmail.com'; // your Gmail
            $mail->Password   = 'joriikysgfbptiuv';            // your App Password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port       = 587;

            // Recipients
            $mail->setFrom('myprojectemails4u@gmail.com', 'Bank IFSC');
            $mail->addAddress($email);

            // Content
            $mail->isHTML(false);
            $mail->Subject = 'Your Bank IFSC PIN';
            $mail->Body    = "Dear Customer,\n\nYour PIN is: $pin\n\nRegards,\nBank IFSC";

            $mail->send();
            $success = "Your PIN has been sent to your registered email.";
        } catch (Exception $e) {
...
<head>
<meta charset="UTF-8">
<title>Customer Login | Bank IFSC Finder</title>
...
/* Header - EXACT same as adminlogin.php */
.header {
    display: flex;
    align-items: center;
    padding: 10px 20px;
...
/* Dropdown Menu - EXACT same as adminlogin.php */
.dropdown {
    position: absolute;
    top: 65px;
    left: 20px;
...
.login-btn::before, .forgot-btn::before {
    content: '';
    position: absolute;
    top: 0;
    left: -100%;
...
.login-btn:hover::before, .forgot-btn:hover::before {
    left: 100%;
}
...
.login-btn:active, .forgot-btn:active {
    transform: translateY(0);
}
    ...
    <div class="login-container">
        <div class="login-card">
            ...
            <div class="login-header">
                <h2><i class="fa fa-users"></i> Customer Login</h2>
            ...
            <form method="post">
                ...
                <div class="button-container">
                    ...
                    <button class="forgot-btn" type="submit" name="forgot">
...
document.addEventListener('DOMContentLoaded', function() {
    const currentPage = 'customerlogin.php';
